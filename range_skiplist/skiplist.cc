...
TOID(leaf_node) SkipList::make_leafNode(int min_val)
{
  ...
  POBJ_ALLOC(pop, &newHT, KuKuTable, size_of(KukuTable), NULL, NULL);
  ...
  POBJ_ALLOC(pop, &newBF, bloom_filter, size_of(bloom_filter), NULL, NULL);

	TOID(leaf_node) leafnode;
  POBJ_ALLOC(pop, &leafnode, leaf_node, size_of(leaf_node), NULL, NULL);
...
SkipList::SkipList(int max_level)
	...
	D_RW(leaf_head) = SkipList::make_leafNode(-1); // head points NULL leaf node and its min value is -1
	index_head = SkipList::make_indexNode(max_level,-1,leaf_head); // head points NULL leaf node and its min value is -1
	D_RW(leaf_tail) = SkipList::make_leafNode(-1); // head points NULL leaf node and its min value is -1
  ...
  for (int i = 0; i < max_level; i++) {   
    ...
    POBJ_ALLOC(pop, &leaf_head, lead_node, size_of(leaf_node), NULL, NULL);
    POBJ_ALLOC(pop, &leaf_tail, leaf_node, size_of(lead_node), NULL, NULL); 
...
void SkipList::insert(int key, const std::string& value) {
  ...
  POBJ_ALLOC(pop, &new_leaf, leaf_node, size_of(leaf_node), NULL, NULL);
	}
  else
  {
  }
...
*/
    ...
    D_RW(D_RW(new_leaf)->leaf_forward) = update[0]->D_RW(D_RW(leaf)->leaf_forward);
    update[0]->D_RW(D_RW(leaf)->leaf_forward) = new_leaf;
