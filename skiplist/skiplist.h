...
struct leaf_node
{
	...
	~leaf_node();
	int min;
	vector<shared_ptr<leaf_node>> leaf_forward;
	bloom_parameters parameters;
	bloom_filter BF(parameters);
	int log_table_size = 8; // default cuckoo hashing 
    size_t stash_size = 2;
...
private:
	...
	static std::unique_ptr<leaf_Node> make_leafNode(uint64_t key,
		const std::string& value); // it should be modified for storing BF
